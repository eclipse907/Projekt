...
class L1SmoothLoss():
  ...
  def forward(self):
    ...
    lossAllWeights = np.where(np.abs(self.weights) < 1/self.weight_decay, self.weight_decay*0.5* np.square(self.weights), np.abs(self.weights)-0.5/self.weight_decay)
  ...
  def backward_params(self):
    ...
    gradAllWeights =  np.where(np.abs(self.weights) < 1/self.weight_decay, self.weight_decay * self.weights, np.sign(self.weights))
